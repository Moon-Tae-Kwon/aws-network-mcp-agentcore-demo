...
def agent_invocation(payload, context):
    ...
    with MCPClient(mcp_factory) as mcp:
        agent = Agent(
            ...
            system_prompt="You are an expert AWS network engineer. Use the provided network tools to analyze issues."
        )
        prompt = payload.get("prompt", "")
        if not prompt:
            prompt = f"Ticket: {payload.get('type','')}. Fields: {payload.get('fields',{})}. Analyze this."
        result = agent(prompt)
        msg = result.message
